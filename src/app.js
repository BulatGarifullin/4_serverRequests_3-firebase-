...
export const App = () => {
	...
	const onTodoSave = (todoId) => {
		...
		if (todoId === NEW_TODO_ID) {
			createTodo({ title, completed }).then((id) => {
				let updateTodos = setTodoInTodos(todos, { id: NEW_TODO_ID, isEditing: false });
	...
	return (
		<div className={styles.app}>
			<ControlPanel onTodoAdd={onTodoAdd} onSearch={setSearchPhrase} onSorting={setIsAlphabetSorting} />
			<div>
